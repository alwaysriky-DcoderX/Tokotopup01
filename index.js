...
function requireLogin(req, res, next) {
  if (!req.session || !req.session.userId) {
    ...
    // kalau buka page biasa
    return res.redirect("/auth/login");
  }

  // custom admin boleh lewat
...
app.post("/profile/whitelist-ip/add", requireLogin, async (req, res) => {
  try {
    ...
    // reset 0.0.0.0 kalau tambah IP lain
    ...
    if (user.whitelistIp.includes(ip)) {
      return res.status(400).json({
        success: false,
        message: "Alamat IP sudah ada di whitelist",
      });
    }
    ...
    return res.json({
      success: true,
      message: "Alamat IP berhasil ditambahkan",
      whitelistIp: user.whitelistIp,
    });
  } catch (err) {
