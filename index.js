  ...
  try {
    ...
    return res.json({
      ...
      user: {
        ...
        whitelistIp: Array.isArray(user.whitelistIp) ? user.whitelistIp : [],
        historyOrder: user.historyOrder || [],
        historyDeposit: user.historyDeposit || [],
      },
    });
  } catch (error) {
    console.error("GET /profile/users error:", error);
    return res.status(500).json({
      success: false,
      message: "Internal server error",
    });
...
app.post("/profile/whitelist-ip/add", requireLogin, async (req, res) => {
  try {
    ...
    if (!Array.isArray(user.whitelistIp)) {
      user.whitelistIp = [];
    }

    // hapus 0.0.0.0 jika tambah IP lain
    if (user.whitelistIp.includes("0.0.0.0") && ip !== "0.0.0.0") {
      user.whitelistIp = [];
    }
    ...
    if (user.whitelistIp.includes(ip)) {
      return res.status(400).json({
        success: false,
        message: "IP sudah ada di whitelist"
      });
    }
...
app.post("/profile/whitelist-ip/remove", requireLogin, async (req, res) => {
  try {
    ...
    user.whitelistIp = (user.whitelistIp || []).filter(
      (item) => item !== ip
    );
