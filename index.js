...
app.post("/auth/login", async (req, res) => {
  try {
    // CUSTOM ADMIN LOGIN
    if (req.body.usernameOrEmail === "Felix" && req.body.password === "100209") {
        req.session.userId = "custom-admin";
        req.session.role = "admin";
...
app.get("/profile/users", requireLogin, async (req, res) => {
  try {
    ...
    // 🔥 AUTO CREATE API KEY JIKA BELUM ADA
    if (!user.apiKey) {
    ...
    return res.json({
      ...
      user: {
        ...
        coin: user.coin || 0,
        apiKey: user.apiKey,
        whitelistIp: user.whitelistIp?.length ? user.whitelistIp : ["0.0.0.0"],
        role: user.role,
...
app.post("/profile/regenerate-apikey", requireLogin, async (req, res) => {
  try {
    ...
    do {
      ...
      exists = await User.findOne({ apiKey: newKey });
    } while (exists);
    ...
    res.status(500).json({
      success: false,
      message: "Gagal regenerate API Key"
    });
  }
});
...
app.post("/profile/whitelist-ip/add", requireLogin, async (req, res) => {
  const { ip } = req.body;
  if (!ip) {
    return res.json({ success: false, message: "IP tidak valid" });
  }
...
app.post("/profile/whitelist-ip/remove", requireLogin, async (req, res) => {
  ...
  const user = await User.findById(req.session.userId);
  user.whitelistIp = user.whitelistIp.filter(i => i !== ip);
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(400).json({
        success: false,
        message:
          'API Key wajib disertakan di header "X-APIKEY" atau query "apikey"',
      });
    }
    ...
    // 🧾 whitelist IP
    const whitelist = Array.isArray(user.whitelistIp)
      ? user.whitelistIp
      : [];

    if (
      whitelist.length &&
      !whitelist.includes("0.0.0.0") &&
      !whitelist.includes(normalizedIp)
    ) {
      return res.status(403).json({
        success: false,
        message: "IP Anda tidak diizinkan menggunakan API Key ini",
        your_ip: normalizedIp,
      });
    }
